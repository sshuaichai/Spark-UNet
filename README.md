...
Interaction factors into three controllable decisions: **where** (task-conditioned prior + PGTS), **how many** (stage-wise hard Top-K budget), and **what** (MSBA multi-source aggregation with gated residual write-back). Unselected sites keep the CNN base—no token completion or sparse-to-dense reprojection. The same Spark Block mounts on two convolutional hosts, **Spark–U-Net** and **Spark–ResEnc-UNet**. The host keeps local convolution. TAE refines each guided stage along the axes, and WinMHSA3D is the block's windowed branch, run beside MSBA. Default ranking is **Lock-Pr**: the prior and the learnable score share one hard Top-K list. Training and comparison use a matched 3d fullres protocol; outputs stay dense and inference stays sliding-window.
